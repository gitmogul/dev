...
////////////////////////////////////////////////////////////////////////////////
static ssize_t power_show( struct device_driver* p_driver, char *buf )
{
    pr_info( "%s\n", __FUNCTION__ );
    return 0;
}
static ssize_t power_store( struct device_driver* p_driver, const char* buf, size_t count )
{
    pr_info( "%s, count %lu\n", __FUNCTION__, (unsigned long)count );
    return count;
}
static DRIVER_ATTR_RW( power );
...
static int __init shadow_driver_init( void )
{
    ...
    ret = platform_driver_register( &shadow_device_driver );
    if( ret == 0 ) {
        ret = driver_create_file( &shadow_device_driver.driver, &driver_attr_power );
        if( ret == 0 ) {
            // iterate through the device list and register the devices    
            for( index = 0; index < NUM_SHADOW_DEVICES; index++ ) {
                ret = platform_device_register( &shadow_device[index] );
                if( ret != 0 ) {
                    driver_remove_file( &shadow_device_driver.driver, &driver_attr_power );
